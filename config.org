...
emacs keybindings are also inconvenient. Rebind these to something useful
...
=eshell= is a built-in shell + terminal emulator that can be used
cross-platform. Since it works very well with =evil= out of the box we
